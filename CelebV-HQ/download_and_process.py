...
def download(video_path, ytb_id, proxy=None):
    ...
    if not os.path.exists(video_path):
        down_video = " ".join([
            "yt-dlp",
            proxy_cmd,
            '-f', "'bestvideo[ext=mp4]+bestaudio[ext=wav]/bestvideo+bestaudio'",
...
def job(data):
    for vid_id, save_vid_name, time, bbox in data:
...
if __name__ == '__main__':
    ...
    raw_vid_root = './raw/'  # download raw video path
    processed_vid_root = './processed/'  # processed video path
    ...
    p = ProcessPoolExecutor(max_workers=16)

    print("Loading annotation...")
    data = list(load_data(json_path))
    print("Loaded !")
